...
class OandaConnector:
    ...
    def place_oco_order(self, instrument: str, entry_price: float, stop_loss: float, 
        ...
        try:
            # Idempotency: skip if an equivalent pending limit already exists
            try:
                pending = self.get_orders(state="PENDING")
                def _matches(o: Dict[str, Any]) -> bool:
                    try:
                        return (
                            (o.get("type") == "LIMIT") and
                            (o.get("instrument") == instrument) and
                            (float(o.get("price", 0)) == float(entry_price)) and
                            (int(o.get("units", 0)) == int(units))
                        )
                    except Exception:
                        return False
                match = next((o for o in pending if _matches(o)), None)
                if match:
                    existing_id = match.get("id") or match.get("clientExtensions",{}).get("id")
                    self.logger.info(
                        f"Idempotent skip: pending LIMIT already exists for {instrument} @ {entry_price} units {units} (id={existing_id})"
                    )
                    return {
                        "success": True,
                        "duplicate": True,
                ...
                # Non-fatal; proceed with placement
                pass
        ...
        try:
            ...
            if self.environment == "live":
                ...
                signature = f"oco:{instrument}|{float(entry_price):.5f}|{int(units)}"
                order_data = {
                    "order": {
    ...
    def get_account_info(self) -> Dict[str, Any]:
        """Get account information including balance, margin, NAV
        
        Returns:
            Dict with keys: balance, NAV, margin_available, margin_used, unrealized_pl
        """
        try:
            endpoint = f"/v3/accounts/{self.account_id}/summary"
            resp = self._make_request("GET", endpoint)
            if resp.get("success"):
                account_data = resp.get("data", {}).get("account", {})
                return {
                    "NAV": float(account_data.get("NAV", 2000.0)),
                    "balance": float(account_data.get("balance", 2000.0)),
                    "margin_available": float(account_data.get("marginAvailable", 2000.0)),
            ...
            self.logger.warning(f"Failed to fetch account info: {e}")
