...
class CoinbaseConnector:
    ...
    def __init__(self, pin: int = None, environment: str = None):
        ...
        # Global pause toggle (default: paused per user directive)
        self.paused = str(os.getenv("COINBASE_ENABLED", "false")).lower() in ("0", "false", "no", "off", "")
    ...
    self.environment = environment or "sandbox"
        ...
        # If paused, do not load creds or validate
        if self.paused:
            self.logger.info("CoinbaseConnector is paused (COINBASE_ENABLED=false). Skipping credential load and validation.")
            # Initialize minimal fields to avoid attribute errors
            self.api_base = "https://api.coinbase.com"
            self.api_key = None
            self.api_secret = None
            self.passphrase = None
            # Still set tracking structures for uniform API
            self.request_times = []
        ...
        # Load API credentials from environment when not paused
    ...
    # Validate connection when not paused
    ...
    def place_oco_order(self, product_id: str, entry_price: float, stop_loss: float, 
        ...
        if self.paused:
            # Short-circuit when paused
            return {
                "success": False,
                "error": "Coinbase trading is paused (COINBASE_ENABLED=false)",
                "broker": "Coinbase",
    ...
    def _make_request(self, method: str, endpoint: str, data: Dict = None) -> Dict[str, Any]:
        ...
        # Build auth headers (gracefully fail if not supported)
        try:
            headers = self._get_headers(method, endpoint, body)
        except AttributeError:
            # Backward compatibility: if headers method not present, return a clear error
            return {
                "success": False,
                "error": "LIVE API credentials not configured",
                "latency_ms": 0,
    ...
    def _get_headers(self, method: str, endpoint: str, body: str) -> Dict[str, str]:
        """
        Build Coinbase API authentication headers.
        Supports Coinbase Advanced Trade (api.coinbase.com) and Coinbase Exchange style HMAC.

        Notes:
        - For safety in this workspace, live trading is not enabled by default. This method only
          constructs headers; upstream guards decide whether to call live endpoints.
        - If the API secret appears to be PEM-formatted, we will fall back to raw bytes HMAC which
          will likely fail authentication gracefully (HTTP 401) rather than crash.
        """
        timestamp = str(int(time.time()))
        method_up = method.upper()

        # Ensure endpoint is a path (not full URL) for signature prehash
        path = endpoint if endpoint.startswith('/') else f'/{endpoint}'
        prehash = f"{timestamp}{method_up}{path}{body or ''}"

        secret = self.api_secret or ""

        # Try to base64-decode secret (Coinbase Exchange style). If it fails, use raw bytes.
        secret_bytes: bytes
        try:
            # Strip PEM markers if present
            if "-----BEGIN" in secret:
                # Extract base64 payload between PEM markers as a best effort
                lines = [ln.strip() for ln in secret.splitlines() if not ln.startswith('-----') and ln.strip()]
                secret_b64 = ''.join(lines)
                secret_bytes = base64.b64decode(secret_b64)
            else:
                secret_bytes = base64.b64decode(secret)
        except Exception:
            # Fallback to raw bytes (will still produce deterministic HMAC)
        ...
        # Optional version header for Advanced Trade
        headers['CB-VERSION'] = '2023-10-01'
