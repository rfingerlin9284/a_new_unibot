...
class Position:
    ...
    @property
    def base(self) -> str:
        b, _ = split_symbol(self.symbol)
        return b

    @property
    def quote(self) -> str:
        _, q = split_symbol(self.symbol)
        return q
...
# ---- Tunables ----
PIP_BE_THRESHOLD = 25.0     # >=25 pips -> breakeven
BE_OFFSET_PIPS = 5.0
R_FOR_BE = 1.0
MINOR_TIME_HRS = 3.0
MAJOR_TIME_HRS = 6.0
...
# Profit Autopilot thresholds
S2_START_PIPS = 40.0        # begin trailing
S3_START_PIPS = 60.0        # tighten trailing
TRAIL_D2_PIPS = 18.0
TRAIL_D3_PIPS = 12.0
BOOTSTRAP_SL_PIPS = 20.0
GIVEBACK_PCT = 0.40         # 40% of peak
...
def enforce_bootstrap_sl(p: Position) -> Optional[Dict]:
    ...
    return {
        ...
        "symbol": p.symbol,
        "new_sl": round(new_sl, 10),
        "why": "bootstrap_hard_SL_OCO"
    }


def trailing_actions(p: Position, st: Dict) -> List[Dict]:
    """Stage machine + peak-giveback trailing."""
    acts: List[Dict] = []
    pid = p.id
    peak = float(_get(st, pid, "peak_pips", 0.0))
    stage = int(_get(st, pid, "stage", 0))
    ...
    if peak >= S2_START_PIPS:
        ...
        if p.pips_open <= floor_pips:
            acts.append({
                ...
                "position_id": p.id,
                "symbol": p.symbol,
                "why": f"peak_giveback_{int(GIVEBACK_PCT * 100)}pct"
            })
            return acts
    ...
    if stage >= 2:
        ...
        # For long: SL = current - gap; For short: SL = current + gap
        target_sl = p.current_price - (gap_pips * p.pip_size * p.direction * -1)
        if p.stop_loss is None or (p.direction * (target_sl - p.stop_loss)) > 0:
            acts.append({
                "type": "modify_sl",
                "position_id": p.id,
                "symbol": p.symbol,
                "new_sl": round(target_sl, 10),
                "why": f"trail_stage_{stage}_gap_{gap_pips}p"
            })
    return acts
...
def tl_dr_actions(positions: List[Position], acct: AccountState, now_utc: Optional[datetime] = None) -> List[Dict]:
    ...
    if acct.margin_utilization > MARGIN_CAP:
        acts.append({
            "type": "advice",
            "why": "margin>35%",
            "suggestion": "reduce_margin_to<=35%"
        })
    return acts
