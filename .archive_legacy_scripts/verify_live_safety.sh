...
if grep -r "simulate\|simulated\|fake\|mock\|demo" /home/ing/RICK/R_H_UNI/live/ 2>/dev/null; then
    echo "❌ DANGEROUS: Simulation code found in live folder!"
    echo "Deactivating live trading..."
    rm -f /home/ing/RICK/R_H_UNI/live/config.json
    exit 1
fi

# Verify risk parameters
if [ -f /home/ing/RICK/R_H_UNI/live/config.json ]; then
    mode=$(jq -r '.mode' /home/ing/RICK/R_H_UNI/live/config.json)
    real_money=$(jq -r '.real_money' /home/ing/RICK/R_H_UNI/live/config.json)
...
if [ -f /home/ing/RICK/R_H_UNI/.upgrade_toggle ]; then
    toggle_status=$(cat /home/ing/RICK/R_H_UNI/.upgrade_toggle)
    echo "✅ Upgrade toggle status: $toggle_status"
else
    echo "⚠️ No upgrade toggle found - creating with OFF status"
    echo "OFF" > /home/ing/RICK/R_H_UNI/.upgrade_toggle
fi
